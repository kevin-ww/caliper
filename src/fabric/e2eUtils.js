...
function installChaincode(org, chaincode) {
	...
	let orderCaRootsPath = path.join(__dirname, rootPath, caRootsPath)
	console.log("reading orderCaRootsfrom "+orderCaRootsPath)
	let data = fs.readFileSync(orderCaRootsPath);
	...
	for (let key in ORGS[org]) {
		if (ORGS[org].hasOwnProperty(key)) {
			if (key.indexOf('peer') === 0) {
				let peerTlsCaCertPath = path.join(__dirname, rootPath, ORGS[org][key]['tls_cacerts'])
				console.log("reading peerTlsCaCert from "+peerTlsCaCertPath)
				let data = fs.readFileSync(peerTlsCaCertPath);
